...
import { Plus, MoreHorizontal, Search, Building, User, Hash } from 'lucide-react';
...
const UnitManagement = () => {
  ...
  useEffect(() => {
    const fetchUnits = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${import.meta.env.VITE_BASE_URL}/unit`);
        if (!response.ok) {
  ...
  const filteredUnits = units.filter(unit =>
    ...
    unit.pic.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const currentData = filteredUnits.slice(startIndex, startIndex + itemsPerPage);
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-blue-100 p-8 mb-8">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900 mb-2">Master Unit</h1>
                <p className="text-gray-600 text-lg">Kelola data unit dengan mudah dan efisien</p>
              </div>
            </div>
            <button className="bg-gradient-to-r from-[#000080] to-blue-600 hover:from-blue-700 hover:to-blue-800 text-white px-6 py-3 rounded-xl flex items-center gap-3 shadow-lg hover:shadow-xl transform hover:-translate-y-1 transition-all duration-200">
              <Plus size={20} />
              <span className="font-medium">Tambah Unit</span>
            </button>
          </div>
        </div>
          ...
          <div>
            {currentData.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="text-gray-500 text-lg">Tidak ada data unit yang ditemukan</p>
              </div>
            ) : (
              currentData.map((unit, index) => (
                <div key={unit.id} className="grid grid-cols-12 gap-4 px-8 py-6 border-b border-gray-100 last:border-b-0 hover:bg-blue-50 transition-all duration-200 group">
